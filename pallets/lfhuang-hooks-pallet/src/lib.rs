...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn on_initialize(n: BlockNumberFor<T>) -> Weight {
			log::info!(target: "lfhuang-hooks", "++++++++++++ on_initialize, block number is {:?}", n);
			Weight::zero()
		}
		// 在区块完成finalize的时候调用。
		fn on_finalize(n: BlockNumberFor<T>) {
			log::info!(target: "lfhuang-hooks", "------------ on_finalize, block number is {:?}", n);
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_param_bigger_than_100(origin: OriginFor<T>, param: u32) -> DispatchResult {
			...
			log::info!(target: "lfhuang-hooks", "set param bigger then 100");

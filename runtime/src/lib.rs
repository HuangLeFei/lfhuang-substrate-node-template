...
impl lfhuang_pallet_template::Config for Runtime {
...
impl lfhuang_hooks_template::Config for Runtime {
	type RuntimeEvent = RuntimeEvent; //我们上面的定义中只有一个关联类型Event，
								  //在此处进行指定，等号右边的Event实际上是
								  //frame system中的Event，此处不需要深究，
								  //可以理解为在runtime中已经定义好的一种具体的类型。
}
impl lfhuang_config_template::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
}
impl lfhuang_config2_template::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type AccountIdType = u32; //指定具体的类型
...
impl frame_system::offchain::AppCrypto<<Signature as Verify>::Signer, Signature> for MyAuthorityId {
	type RuntimeAppPublic = lfhuang_ocw_signed_pallet_template::crypto::Public;
	type GenericSignature = sp_core::sr25519::Signature;
	type GenericPublic = sp_core::sr25519::Public;
}

// 实现pallet中的config配置
impl lfhuang_ocw_signed_pallet_template::Config for Runtime {
...
// 实现pallet中的config配置
impl lfhuang_test_pallet_template::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type UserType = u32;
}
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		// 定义一个变量LfhuangPallet，该变量是这个lfhuang_pallet_template类型
		LfhuangPallet: lfhuang_pallet_template,
		// 定义一个变量LfhuangHooksPallet，该变量是这个lfhuang_hooks_template类型
		LfhuangHooksPallet: lfhuang_hooks_template,
		// 定义一个变量LfhuangConfigPallet，该变量是这个lfhuang_config_template类型
